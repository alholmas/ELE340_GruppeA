...
{   
  ...
  // Slipper til periferklokke til GPIOB og ADC3
  LL_AHB1_GRP1_EnableClock(LL_AHB1_GRP1_PERIPH_GPIOB);
  LL_AHB1_GRP1_EnableClock(LL_AHB1_GRP1_PERIPH_ADC34);

  // Konfigurerer PB1 til ADC3 kanal 1
  GPIO_InitStruct.Pin = LL_GPIO_PIN_1;
  GPIO_InitStruct.Mode = LL_GPIO_MODE_ANALOG;
  GPIO_InitStruct.Pull = LL_GPIO_PULL_NO;
  LL_GPIO_Init(GPIOB, &GPIO_InitStruct);

  // Konfigurerer interupt for ADC3
  NVIC_SetPriority(ADC3_IRQn, NVIC_EncodePriority(NVIC_GetPriorityGrouping(),0, 0));
  NVIC_EnableIRQ(ADC3_IRQn);

  // Setter opp ADC3
  ADC3->CFGR = ADC_CFGR_EXTEN_0 | (ADC_CFGR_EXTSEL_0 | ADC_CFGR_EXTSEL_2 | ADC_CFGR_EXTSEL_3); // TIM7 TRGO rising edge trigger
  ADC3->CFGR &= ~(ADC_CFGR_CONT | ADC_CFGR_RES | ADC_CFGR_ALIGN); // 12-bit oppløsning, høyrejustert, single conversion mode
  ADC3->IER |= ADC_IER_EOCIE; // Enable end of regular conversion interrupt flag
  

  // Setter først reset state for ADC3, ADVREGEN = 00
  ADC3->CR &= ~ADC_CR_ADVREGEN;
  // Setter enable state, ADVREGEN = 01 
  ADC3->CR |= ADC_CR_ADVREGEN_0;
  
  // Vent på at spenningsregulatoren skal stabilisere seg
  ...
  ADC3->SQR1 = (ADC3->SQR1 & ~ADC_SQR1_SQ1) | (1UL << ADC_SQR1_SQ1_Pos);
